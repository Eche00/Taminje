...
function Header() {
    ...
    return (
        <div className="bg-white text-black md:py-3 py-2  fixed top-0 left-0 w-full z-50">
            {/* container  */}
            <section className="flex items-center justify-center gap-2 md:w-[90%] w-[98%] mx-auto">
                {/* Logo   */}
                <div className="flex flex-1 items-center relative ">
                    <span className="flex items-center w-full rounded-lg px-4 md:text-2xl text-xl font-extrabold text-[#33A259] tracking-wide">
                        <Link
                            href="/"
                            className=" border-2 border-[#33A259] rounded-full p-2 mr-2 flex items-center justify-center"
                        >
                    ...
                    <nav className="w-fit  flex items-center gap-4 py-3 px-6 rounded-lg">
                        {publicItems.slice(0, 3).map((item) => (
                            <Link
                                ...
                                className={
                                    item.link === pathname
                                        ? "text-[#33A259] text-[16px] font-semibold duration-300 border-b-2 border-b-[#33A259]"
                ...
                <div className="flex md:hidden  items-center justify-end gap-5 ">
                    <button
                        className=" text-black  cursor-pointer w-fit pr-4"
            ...
            <AnimatePresence>
                {openMenu && (
                    <section
                        ...
                        className="fixed top-17 left-0 w-full h-screen bg-black/50 mx-auto flex  justify-end md:hidden"
                    >
                        <motion.aside
                        ...
                        >
                            {publicItems.slice(0, 3).map((item) => (
                                <Link
                                    ...
                                    className={
                                        pathname === item.link
                                            ? "text-white font-bold bg-[#33A259] py-3 px-4  rounded-lg flex items-center gap-2"
                                            : "text-black py-3 px-4  rounded-lg hover:bg-[#33A259]/10 flex items-center gap-2"
                                    }
                                >
                                    {item.name}
                                </Link>
